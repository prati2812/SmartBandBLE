...
AlarmSlotSched alarmSched[MAX_ALARMS];
...
void clearAlarmsFromFlash() {
  ...
  memset(alarmDB, 0, sizeof(alarmDB));
  for (int i = 0; i < MAX_ALARMS; i++) {
    alarmSched[i].state = SLOT_IDLE;
    alarmSched[i].lastFiredMinute = -1;
    alarmSched[i].motorRunning = false;
  }
  motorOff();
...
void startAlarmFiring(int slot) {
  ...
  AlarmSlotSched &s = alarmSched[slot];
  Serial.printf("[SCHED] *** ALARM FIRING: %s @ %s ***\n", a.alarm_id, a.alarm_time);

  s.state        = SLOT_FIRING;
  s.alarmStartMs = millis();
  s.pulseIndex   = 0;
  s.motorRunning = false;
  s.motorOnUntil = 0;
  s.motorOffUntil= millis();
}

void stopAlarm(int slot, const char* reason) {
  AlarmSlotSched &s = alarmSched[slot];
  s.state        = SLOT_IDLE;
  s.motorRunning = false;
  Serial.printf("[SCHED] Alarm %d stopped (%s).\n", slot, reason);
}
...
void tickVibrationEngine(int slot) {
  ...
  if (!s.motorRunning) {
    if (now >= s.motorOffUntil) {
      if (s.pulseIndex >= patLen) s.pulseIndex = 0;
      motorOn(a.intensity);
      s.motorRunning = true;
      s.motorOnUntil = now + pulses[s.pulseIndex].on_ms;
    }
  } else {
    if (now >= s.motorOnUntil) {
      motorOff();
      s.motorRunning  = false;
      s.motorOffUntil = now + pulses[s.pulseIndex].off_ms;
      s.pulseIndex++;
    }
  }
}
...
void tickAlarmScheduler() {
  ...
  // 1. Process active alarms that are currently firing
  int activeFiringSlot = -1;
  for (int i = 0; i < MAX_ALARMS; i++) {
    if (alarmSched[i].state == SLOT_FIRING) {
      activeFiringSlot = i;
      break; // Prioritize the first triggered slot
    }
  }

  if (activeFiringSlot != -1) {
    AlarmSlotSched &s = alarmSched[activeFiringSlot];
    Alarm &a = alarmDB[activeFiringSlot];
    
    // Set alarmStartMs if it's starting for the first time in this cycle
    if (s.alarmStartMs == 0) {
      s.alarmStartMs = now;
      s.pulseIndex = 0;
      s.motorRunning = false;
    ...
    unsigned long elapsed = (now - s.alarmStartMs) / 1000UL;
    if (elapsed >= (unsigned long)a.duration_sec) {
      if (a.snooze > 0) {
        motorOff();
        s.state         = SLOT_SNOOZED;
        s.snoozeUntilMs = now + ((unsigned long)a.snooze * 60000UL);
        s.motorRunning  = false;
        Serial.printf("[SCHED] Alarm %d duration elapsed. Snoozed for %d min.\n", activeFiringSlot, a.snooze);
      } else {
        stopAlarm(activeFiringSlot, "duration elapsed, no snooze");
        motorOff();
      }
    } else {
      tickVibrationEngine(activeFiringSlot);
    }
  }

  // 2. Check for snoozed alarms whose timers have expired
  for (int i = 0; i < MAX_ALARMS; i++) {
    AlarmSlotSched &s = alarmSched[i];
    if (s.state == SLOT_SNOOZED) {
      if (now >= s.snoozeUntilMs) {
        Serial.printf("[SCHED] Snooze expired for slot %d — re-firing alarm.\n", i);
        s.state        = SLOT_FIRING;
        s.alarmStartMs = 0; // Defer start until it gains control of the motor
      }
    }
  }

  // 3. Check for new triggers
  ...
  for (int i = 0; i < MAX_ALARMS; i++) {
    ...
    if (!a.active || !a.enabled) {
      // Ensure slot is reset if alarm is disabled or deleted
      if (s.state != SLOT_IDLE) {
        s.state = SLOT_IDLE;
        s.motorRunning = false;
        if (i == activeFiringSlot) {
          motorOff();
        }
      }
      continue;
    }
    if (s.lastFiredMinute == minuteToken) continue;
    if (strcmp(a.alarm_time, currentHHMM) != 0) continue;
    if (!dayMatchesRepeat(a.repeat, todayName)) continue;

    s.lastFiredMinute = minuteToken;
    s.state           = SLOT_FIRING;
    s.alarmStartMs    = 0; // Defer start until motor is free
    Serial.printf("[SCHED] Alarm %d triggered for HH:MM = %s\n", i, a.alarm_time);
  }
}
...
void sendDeviceStatusUpdate() {
  ...
  const char* activeAlarmId = nullptr;
  for (int i = 0; i < MAX_ALARMS; i++) {
    if (alarmSched[i].state == SLOT_FIRING) {
      stateStr = "firing";
      activeAlarmId = alarmDB[i].alarm_id;
      break;
    }
  }
  if (strcmp(stateStr, "idle") == 0) {
    for (int i = 0; i < MAX_ALARMS; i++) {
      if (alarmSched[i].state == SLOT_SNOOZED) {
        stateStr = "snoozed";
        activeAlarmId = alarmDB[i].alarm_id;
        break;
      }
  ...
  if (activeAlarmId != nullptr) {
    doc["active_alarm_id"] = activeAlarmId;
  }
...
void writeAlarmFromJson(JsonObjectConst obj) {
  ...
  // Reset scheduler state for this slot
  alarmSched[slot].state = SLOT_IDLE;
  alarmSched[slot].lastFiredMinute = -1;
  alarmSched[slot].motorRunning = false;
}
...
void handleIncomingJson(const String& jsonString) {
  ...
  if (cmd == "test_vibration") {
    ...
    bool stopped = false;
    for (int i = 0; i < MAX_ALARMS; i++) {
      if (alarmSched[i].state == SLOT_FIRING || alarmSched[i].state == SLOT_SNOOZED) {
        alarmSched[i].state = SLOT_IDLE;
        alarmSched[i].motorRunning = false;
        stopped = true;
      }
    ...
    if (stopped) {
      motorOff();
      Serial.println("[SCHED] All alarms dismissed.");
    }
  }
  else if (cmd == "snooze_alarm") {
    int minutes = doc["minutes"] | 5;
    bool snoozedAny = false;
    for (int i = 0; i < MAX_ALARMS; i++) {
      if (alarmSched[i].state == SLOT_FIRING) {
        int alarmSnooze = minutes;
        if (!doc["minutes"].is<int>()) {
          alarmSnooze = alarmDB[i].snooze > 0 ? alarmDB[i].snooze : 5;
        }
        alarmSched[i].state = SLOT_SNOOZED;
        alarmSched[i].snoozeUntilMs = millis() + ((unsigned long)alarmSnooze * 60000UL);
        alarmSched[i].motorRunning = false;
        snoozedAny = true;
        Serial.printf("[SCHED] Alarm %d snoozed for %d minutes.\n", i, alarmSnooze);
      }
    }
    if (snoozedAny) {
      motorOff();
